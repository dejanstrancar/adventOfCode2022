...
func createOperation(operator rune, value int, modifier int) func(int) int {
...
func Challenge11Part1(inputFile string) int {
	...
	for sc.Scan() {
		...
		newMonkey.operation = createOperation(operator, value, 1)
	...
	return highestCount * secondHighest
}

func Challenge11Part2(inputFile string) int {
	...
	for sc.Scan() {
		...
		newMonkey.operation = createOperation(operator, value, 3)

		sc.Scan()
		var testingValue int
		fmt.Sscanf(sc.Text(), "  Test: divisible by %d", &testingValue)
		bigLimit *= testingValue
